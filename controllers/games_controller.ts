...
export const newGame = (req: Request, res: Response) => {
    const data = req.body;
    const { organizer, starts_at, ends_at, type, localization, status } = data;
    res.status(201).json({
        message: "Partida criada com sucesso!",
        created: { organizer, starts_at, ends_at, type, localization, status }
    });
}
...
export const putGame = (req: Request, res: Response) => {
    const { id } = req.params; 
    const { organizer, starts_at, ends_at, type, localization, status } = req.body;

    res.status(200).json({
        message: `Partida ${id} atualizada com sucesso!`,
        updated: {
            organizer, 
            starts_at, 
            ends_at,
...
export const deleteGame = (req: Request, res: Response) => {
    ...
    res.status(200).json({
        message: `Partida ${id} excluída com sucesso!`
    });
};
